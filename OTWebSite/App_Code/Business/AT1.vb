...
#Region "AT1000 代碼類別下拉選單"
    Public Shared Sub FillFldName_AT1000(ByVal objDDL As DropDownList, Optional ByVal Type As Bsp.Enums.DisplayType = Bsp.Enums.DisplayType.Full)
        Dim objOV As New AT1
        Try
            Using dt As DataTable = objOV.GetFldName_AT1000()
                With objDDL
                    .Items.Clear()
                    .DataSource = dt
                    .DataTextField = "FullName"
    ...
    Public Function GetFldName_AT1000() As DataTable
        Dim strSQL As New StringBuilder()
        strSQL.AppendLine(" SELECT RTrim(FldName) + '\' + RTrim(Code) AS FldName, CodeCName, RTrim(FldName)+'\'+RTrim(Code) + '-' + CodeCName AS FullName ")
        strSQL.AppendLine(" FROM AT_CodeMap WHERE TabName = 'ATCodeMap_OpenMaintain' ")
        Return Bsp.DB.ExecuteDataSet(CommandType.Text, strSQL.ToString, "AattendantDB").Tables(0)
    End Function
#End Region

#Region "AT1000 代碼下拉選單"
    Public Shared Sub FillCode_AT1000(ByVal objDDL As DropDownList, ByVal FldName As String, ByVal Code As String, Optional ByVal Type As Bsp.Enums.DisplayType = Bsp.Enums.DisplayType.Full)
        Dim objOV As New AT1
        Try
            Using dt As DataTable = objOV.GetCode_AT1000(FldName, Code)
                With objDDL
                    .Items.Clear()
                    .DataSource = dt
                    .DataTextField = "FullName"
    ...
    Public Function GetCode_AT1000(ByVal FldName As String, ByVal Code As String) As DataTable
        Dim strSQL As New StringBuilder()
