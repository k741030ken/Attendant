...
#Region "檢查考核補充說明是否已填寫完成"
    Public Function CheckEmpComment(ByVal CompID As String, ByVal ApplyID As String, ByVal ApplyTime As String, ByVal Seq As String, ByVal GradeYear As String, ByVal GradeSeq As String _
        ...
        If MainFlag = "2" Then
            strSQL.AppendLine(" Left join GradeSignLog GS1 on G.CompID=GS1.CompID and G.GradeDeptID=GS1.ApplyID and G.EmpID=GS1.EmpID and GS1.ApplyTime=GS.ApplyTime and GS1.Seq=" & Seq - 1)
        End If
            ...
            Try
                ...
                For i As Integer = 0 To dt.Rows.Count - 1
                    ...
                    If IsSignNext = "N" Then
                        ...
                        strSQL.AppendLine(" Set Grade = EncryptByKey(Key_GUID('eHRMSDBDES'),'" & dt.Rows(i)("Grade").ToString().Trim & "')")
                        strSQL.AppendLine(" ,LastChgComp = " & Bsp.Utility.Quote(UserProfile.CompID))
                        strSQL.AppendLine(" ,LastChgID = " & Bsp.Utility.Quote(UserProfile.UserID))
                        strSQL.AppendLine(" ,LastChgDate = getdate()")
                        strSQL.AppendLine(" Where CompID = " & Bsp.Utility.Quote(dt.Rows(i)("CompID")))
    ...
    Public Function GS1100SendData(ByVal CompID As String, ByVal ApplyID As String, ByVal ApplyTime As String, ByVal Seq As String, ByVal GradeYear As String, ByVal GradeSeq As String _
        ...
        Using cn As DbConnection = Bsp.DB.getConnection("eHRMSDB")
            ...
            Try
                ...
                strSQL.AppendLine(" ,G.GradeOrder= Case when G.GroupID='3' then GS.GradeOrder else '0' end,G.Grade=Case when G.GroupID='3' then GS.Grade else null end ")  '20160722 wei add
                ...
                If IsSignNext = "Y" Then
                    ...
                    Using dt As DataTable = GetSignEMail(CompID, GradeYear, GradeSeq, ApplyTime, ApplyID, Seq)
                        ...
                        End If
                    End Using
                    Dim strEMailSubject As String = GradeYear & "半年度" & strNextOrganName & "績效考核排序簽核通知"
                    ...
                    If DeptEx = "N" Then
                        ...
                        If Not objHR.IsDataExists("SignLog S Inner Join Organization O On S.CompID=O.CompID And S.ApplyID=O.OrganID", strWhere) Then
                            ...
                            Using dt As DataTable = GetSignOrganName(CompID, ApplyID)
                                ...
                                End If
                            End Using
                            strEMailSubject = GradeYear & "半年度" & strNextOrganName & "績效考核排序簽核通知"
                ...
                Else
                    ...
                    strSQL.AppendLine(" Update G Set G.Grade=GS.Grade,G.GradeOrder=GS.GradeOrder,G.GradeOrderHR=GS.GradeOrder,G.GradeHR=GS.Grade,G.GradeOrder2=GS.GradeOrder,G.Grade2=GS.Grade,G.ScoreValue=GS.ScoreValue,G.Score=GS.Score ")
                    strSQL.AppendLine(" From GradeBase G  left join GradeSignLog GS on GS.CompID=G.CompID and GS.ApplyID=G.GradeDeptID and GS.GradeSeq=G.GradeSeq and GS.EmpID=G.EmpID and G.Online='1' ")
                    strSQL.AppendLine(" Where G.CompID = " & Bsp.Utility.Quote(CompID))
                    strSQL.AppendLine(" And G.GradeYear = " & Bsp.Utility.Quote(GradeYear))
                    strSQL.AppendLine(" And G.GradeSeq = " & Bsp.Utility.Quote(GradeSeq))
...
#Region "GS1100 年度考核(單位主管排序)-下載"
    Public Function GS1100Download(ByVal CompID As String, ByVal ApplyID As String, ByVal ApplyTime As String, ByVal Seq As String, ByVal Status As String, ByVal MainFlag As String _
        ...
        If IsPerformance = "Y" Then
            strSQL.AppendLine(" ,isnull(EP.Fld1,'') as 'Q1\個金業務\房貸撥款量(仟元)',isnull(EP.Fld2,'') as 'Q1\個金業務\信貸撥款量(仟元)',isnull(EP.Fld3,'') as 'Q1\個金業務\總AP(仟點)',isnull(EP.Fld4,'') as 'Q1\個金業務\個金Score Card',isnull(EP.Fld5,'') as 'Q1\個金業務\理財Score Card',isnull(EP.Fld6,'') as 'Q1\理財業務\總AP(仟點)',isnull(EP.Fld7,'') as 'Q1\理財業務\Score Card',isnull(EP.Fld8,'') as 'Q1\法金業務(單位)\總AP(仟點)',isnull(EP.Fld9,'') as 'Q1\法金業務(單位)\單位職系達成率' ")    '20160726 wei modify
            strSQL.AppendLine(" ,isnull(EP.Fld10,'') as 'Q2\個金業務\房貸撥款量(仟元)',isnull(EP.Fld11,'') as 'Q2\個金業務\信貸撥款量(仟元)',isnull(EP.Fld12,'') as 'Q2\個金業務\總AP(仟點)',isnull(EP.Fld13,'') as 'Q2\個金業務\個金Score Card',isnull(EP.Fld14,'') as 'Q2\個金業務\理財Score Card',isnull(EP.Fld15,'') as 'Q2\理財業務\總AP(仟點)',isnull(EP.Fld16,'') as 'Q2\理財業務\Score Card',isnull(EP.Fld17,'') as 'Q2\法金業務(單位)\總AP(仟點)',isnull(EP.Fld18,'') as 'Q2\法金業務(單位)\單位職系達成率' ")   '20160726 wei modify
            strSQL.AppendLine(" ,isnull(EP.Fld19,'') as 'Q3\個金業務\房貸撥款量(仟元)',isnull(EP.Fld20,'') as 'Q3\個金業務\信貸撥款量(仟元)',isnull(EP.Fld21,'') as 'Q3\個金業務\總AP(仟點)',isnull(EP.Fld22,'') as 'Q3\個金業務\個金Score Card',isnull(EP.Fld23,'') as 'Q3\個金業務\理財Score Card',isnull(EP.Fld24,'') as 'Q3\理財業務\總AP(仟點)',isnull(EP.Fld25,'') as 'Q3\理財業務\Score Card',isnull(EP.Fld26,'') as 'Q3\法金業務(單位)\總AP(仟點)',isnull(EP.Fld27,'') as 'Q3\法金業務(單位)\單位職系達成率' ")   '20160726 wei modify
            strSQL.AppendLine(" ,isnull(EP.Fld28,'') as '" & GradeYear & "年度平均\個金業務\房貸撥款量(仟元)',isnull(EP.Fld29,'') as '" & GradeYear & "年度平均\個金業務\信貸撥款量(仟元)',isnull(EP.Fld30,'') as '" & GradeYear & "年度平均\個金業務\總AP(仟點)',isnull(EP.Fld31,'') as '" & GradeYear & "年度平均\個金業務\個金Score Card',isnull(EP.Fld32,'') as '" & GradeYear & "年度平均\個金業務\理財Score Card',isnull(EP.Fld33,'') as '" & GradeYear & "年度平均\理財業務\總AP(仟點)',isnull(EP.Fld34,'') as '" & GradeYear & "年度平均\理財業務\Score Card',isnull(EP.Fld35,'') as '" & GradeYear & "年度平均\法金業務(單位)\總AP(仟點)',isnull(EP.Fld36,'') as '" & GradeYear & "年度平均\法金業務(單位)\單位職系達成率' ") '20160726 wei modify
        End If
        strSQL.AppendLine(" from Personal P inner join GradeBase G on G.GradeYear = " & Bsp.Utility.Quote(GradeYear) & " and P.CompID =G.CompID and P.EmpID = G.EmpID and G.GradeSeq = " & Bsp.Utility.Quote(GradeSeq) & " And G.Online='1'")
        strSQL.AppendLine(" left join Organization O on O.CompID = G.CompID and O.OrganID = G.OrderDeptID")
...
#Region "GS1140 資料查詢"
    Public Function QueryEmpPerformance(ByVal CompID As String, ByVal EmpID As String, ByVal EPYear As String) As DataTable
        ...
        strSQL.AppendLine(" , Fld33, Fld34, Fld35,Fld36 ") '20160726 wei add
...
#Region "GS1180 調整排序-查詢"
    Public Function GS1180Query(ByVal CompID As String, ByVal ApplyID As String, ByVal ApplyTime As String, ByVal Seq As String, ByVal MainFlag As String _
        ...
        Dim strSQL As New StringBuilder()
        strSQL.AppendLine(ConfigurationManager.AppSettings("eHRMSDBDES").ToString)
        strSQL.AppendLine(" Select Case When Left(P.EmpID,2)='LA' Then REPLACE(P.EmpID,'LA','+9') Else REPLACE(P.EmpID,'K','-9') End as EmpID , P.NameN + '/' + P.RankID + '/' + Po.Remark as ShowData")
        strSQL.AppendLine(" from Personal P inner join GradeBase G on G.GradeYear = " & Bsp.Utility.Quote(GradeYear) & " and P.CompID =G.CompID and P.EmpID = G.EmpID and G.GradeSeq = " & Bsp.Utility.Quote(GradeSeq) & " And G.Online='1'")
        strSQL.AppendLine(" left join Organization O on O.CompID = G.CompID and O.OrganID = G.OrderDeptID")
        strSQL.AppendLine(" left join GradeSignLog GS on GS.CompID= " & Bsp.Utility.Quote(CompID) & " and GS.ApplyTime= " & Bsp.Utility.Quote(ApplyTime) & " and GS.ApplyID= G.GradeDeptID and GS.Seq= " & Bsp.Utility.Quote(Seq) & " and P.CompID =GS.CompID and P.EmpID = GS.EmpID and G.GradeSeq = GS.GradeSeq")
        strSQL.AppendLine(" left join Title T on P.CompID = T.CompID and P.RankID = T.RankID and P.TitleID = T.TitleID")
...
#Region "GS1180 調整排序-修改"
    Public Function GS1180Update(ByVal aryEmpID() As String, ByVal CompID As String, ByVal ApplyID As String, ByVal ApplyTime As String, ByVal Seq As String, ByVal GradeSeq As String, ByVal GradeYear As String, ByVal IsSignNext As String, ByVal MainFlag As String) As Boolean
        Dim strSQL As New StringBuilder()
            ...
            Try
                ...
                For intLoop As Integer = 0 To aryEmpID.GetUpperBound(0)
                    If aryEmpID(intLoop) <> "" Then
                        ...
                        strSQL.AppendLine(" and ApplyTime = " & Bsp.Utility.Quote(ApplyTime))
                        'strSQL.AppendLine(" and ApplyID = " & Bsp.Utility.Quote(ApplyID))
                        strSQL.AppendLine(" and EmpID = " & Bsp.Utility.Quote(IIf(aryEmpID(intLoop).Length = "5", "LA" & aryEmpID(intLoop).Right(4), aryEmpID(intLoop).Replace("-9", "K"))))
                        strSQL.AppendLine(" and Seq = " & Bsp.Utility.Quote(Seq))
                        strSQL.AppendLine(" and GradeSeq = " & Bsp.Utility.Quote(GradeSeq) & ";")

                        If MainFlag = "2" Or (MainFlag = "1" And IsSignNext = "N") Then
                            '考績
                            strSQL.AppendLine(" Update GradeBase ")
                            strSQL.AppendLine(" Set GradeOrder = " & intOrder)
                            strSQL.AppendLine(" ,LastChgComp = " & Bsp.Utility.Quote(UserProfile.CompID))
        ...
        Using cn As DbConnection = Bsp.DB.getConnection("eHRMSDB")
            ...
            Try
                ...
                For i As Integer = 0 To dt.Rows.Count - 1
                    ...
                    '考績
                    strSQL.AppendLine(" Update GradeBase ")
                    strSQL.AppendLine(" Set Grade = EncryptByKey(Key_GUID('eHRMSDBDES'),'" & dt.Rows(i)("Grade").ToString().Trim & "')")    '20160722 wei modify
                    strSQL.AppendLine(" ,LastChgComp = " & Bsp.Utility.Quote(UserProfile.CompID))
                    strSQL.AppendLine(" ,LastChgID = " & Bsp.Utility.Quote(UserProfile.UserID))
                    strSQL.AppendLine(" ,LastChgDate = getdate()")
                    strSQL.AppendLine(" Where CompID = " & Bsp.Utility.Quote(dt.Rows(i)("CompID")))
    ...
    Public Function GS1200SendData(ByVal CompID As String, ByVal ApplyID As String, ByVal ApplyTime As String, ByVal Seq As String, ByVal GradeYear As String, ByVal GradeSeq As String _
        ...
        Using cn As DbConnection = Bsp.DB.getConnection("eHRMSDB")
            ...
            Try
                ...
                'Update GradeBase.Grade,GradeOrder
                'strSQL.AppendLine(" Update G Set G.Grade=GS.Grade,G.GradeOrder=GS.GradeOrder,G.GradeOrderHR=GS.GradeOrder,G.GradeHR=G.GradeAdjust,G.GradeOrder2=GS.GradeOrder,G.Grade2=G.GradeAdjust,G.ScoreValue=GS.ScoreValue,G.Score=GS.Score ")
                strSQL.AppendLine(" Update G Set G.Grade=GS.Grade,G.GradeOrder=GS.GradeOrder,G.GradeOrderHR=GS.GradeOrder,G.GradeHR=GS.Grade,G.GradeOrder2=GS.GradeOrder,G.Grade2=GS.Grade,G.ScoreValue=GS.ScoreValue,G.Score=GS.Score ") '20160722 wei modify
                strSQL.AppendLine(" From GradeBase G  left join GradeSignLog GS on GS.CompID=G.CompID and GS.ApplyID=G.GradeDeptID and GS.GradeSeq=G.GradeSeq and GS.EmpID=G.EmpID and G.Online='1' ")
                strSQL.AppendLine(" Where G.CompID = " & Bsp.Utility.Quote(CompID))
                strSQL.AppendLine(" And G.GradeYear = " & Bsp.Utility.Quote(GradeYear))
                strSQL.AppendLine(" And G.GradeSeq = " & Bsp.Utility.Quote(GradeSeq))
