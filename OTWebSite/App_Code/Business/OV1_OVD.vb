Imports Microsoft.VisualBasic

Public Class OV1_OVD
    Public Property OTCompID As String
    Public Property OTEmpID As String
    Public Property OTStartDate As String
    Public Property OTEndDate As String
    Public Property OTSeq As String
    Public Property OTTxnID As String
    Public Property OTSeqNo As String
    Public Property DeptID As String
    Public Property OrganID As String
    Public Property DeptName As String
    Public Property OrganName As String
    Public Property FlowCaseID As String
    Public Property OTStartTime As String
    Public Property OTEndTime As String
    Public Property OTTotalTime As String
    Public Property SalaryOrAdjust As String
    Public Property AdjustInvalidDate As String
    Public Property MealFlag As String
    Public Property MealTime As String
    Public Property OTTypeID As String
    Public Property OTReasonID As String
    Public Property OTReasonMemo As String
    Public Property OTAttachment As String
    Public Property OTFormNO As String
    Public Property OTRegisterID As String
    Public Property OTRegisterDate As String
    Public Property OTStatus As String
    Public Property HolidayOrNot As String
    Public Property OTValidDate As String
    Public Property OTValidID As String
    Public Property OTRejectDate As String
    Public Property OTRejectID As String
    Public Property OTGovernmentNo As String
    Public Property LastChgComp As String
    Public Property LastChgID As String
    Public Property LastChgDate As String
    Public Property OTFromAdvanceTxnId As String
    Public Property AdjustStatus As String
    Public Property AdjustDate As String
    Public Property OverTimeFlag As String
    Public Property ToOverTimeDate As String
    Public Property ToOverTimeFlag As String
    Public Property OTSalaryPaid As String
    Public Property ProcessDate As String
    Public Property OTPayDate As String
    Public Property OTModifyDate As String
    Public Property OTRemark As String
    Public Property KeyInComp As String
    Public Property KeyInID As String
    Public Property HRKeyInFlag As String
    Public Property OTRegisterComp As String
End Class
