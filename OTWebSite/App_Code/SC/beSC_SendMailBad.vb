...
Namespace beSC_SendMailBad
    Public Class Table
        ...
        Public ReadOnly Property Rows() As beSC_SendMailBad.Rows 
            Get
                Return m_Rows
            End Get
        End Property
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Sub Transfer2Row(SC_SendMailBadTable As DataTable)
            For Each dr As DataRow In SC_SendMailBadTable.Rows
    ...
    Public Class Rows
        ...
        Public Sub Add(SC_SendMailBadRow As Row)
            m_Rows.Add(SC_SendMailBadRow)
        End Sub

        Public Sub Remove(SC_SendMailBadRow As Row)
            If m_Rows.IndexOf(SC_SendMailBadRow) >= 0 Then
                m_Rows.Remove(SC_SendMailBadRow)
            End If
        End Sub
    ...
    Public Class Service
        Public Function DeleteRowByPrimaryKey(ByVal SC_SendMailBadRow As Row) As Integer
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()

            strSQL.AppendLine("Delete From SC_SendMailBad")
            strSQL.AppendLine("Where ArrivalDt = @ArrivalDt")
            strSQL.AppendLine("And MailSeqNo = @MailSeqNo")

            dbcmd = db.GetSqlStringCommand(strSQL.ToString())
            db.AddInParameter(dbcmd, "@ArrivalDt", DbType.String, SC_SendMailBadRow.ArrivalDt.Value)
            db.AddInParameter(dbcmd, "@MailSeqNo", DbType.Int32, SC_SendMailBadRow.MailSeqNo.Value)

            return db.ExecuteNonQuery(dbcmd)
        End Function

        public Function DeleteRowByPrimaryKey(ByVal SC_SendMailBadRow As Row, ByVal tran As DbTransaction) As Integer
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()
        ...
        Public Function DeleteRowByPrimaryKey(ByVal SC_SendMailBadRow As Row()) As Integer
            ...
            using cn As DbConnection = db.CreateConnection()
                ...
                Try
                    For Each r As Row In SC_SendMailBadRow
        ...
        Public Function QueryByKey(ByVal SC_SendMailBadRow As Row) As DataSet
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()

            strSQL.AppendLine("Select * From SC_SendMailBad")
            strSQL.AppendLine("Where ArrivalDt = @ArrivalDt")
            strSQL.AppendLine("And MailSeqNo = @MailSeqNo")
        ...
        Public Function QueryByKey(SC_SendMailBadRow As Row, tran As DbTransaction) As DataSet
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()
        ...
        Public Function Update(ByVal SC_SendMailBadRow As Row) As Integer
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()
            Dim strDot As String = String.Empty

            strSQL.AppendLine("Update SC_SendMailBad Set")
            For i As Integer = 0 To SC_SendMailBadRow.FieldNames.Length - 1
                If Not SC_SendMailBadRow.IsIdentityField(SC_SendMailBadRow.FieldNames(i)) AndAlso SC_SendMailBadRow.IsUpdated(SC_SendMailBadRow.FieldNames(i)) AndAlso SC_SendMailBadRow.CreateUpdateSQL(SC_SendMailBadRow.FieldNames(i)) Then
                    strSQL.AppendLine(string.Format("{0}{1} = @{1}", strDot, SC_SendMailBadRow.FieldNames(i)))
                    strDot = ","
                End If
            Next
            ...
            dbcmd = db.GetSqlStringCommand(strSQL.ToString())
            If SC_SendMailBadRow.ArrivalDt.Updated Then db.AddInParameter(dbcmd, "@ArrivalDt", DbType.String, SC_SendMailBadRow.ArrivalDt.Value)
            If SC_SendMailBadRow.MailSeqNo.Updated Then db.AddInParameter(dbcmd, "@MailSeqNo", DbType.Int32, SC_SendMailBadRow.MailSeqNo.Value)
            If SC_SendMailBadRow.SenderID.Updated Then db.AddInParameter(dbcmd, "@SenderID", DbType.String, SC_SendMailBadRow.SenderID.Value)
            If SC_SendMailBadRow.MailAddress.Updated Then db.AddInParameter(dbcmd, "@MailAddress", DbType.String, SC_SendMailBadRow.MailAddress.Value)
            If SC_SendMailBadRow.MailHeader.Updated Then db.AddInParameter(dbcmd, "@MailHeader", DbType.String, SC_SendMailBadRow.MailHeader.Value)
            If SC_SendMailBadRow.GreetWord.Updated Then db.AddInParameter(dbcmd, "@GreetWord", DbType.String, SC_SendMailBadRow.GreetWord.Value)
            If SC_SendMailBadRow.PsDesc.Updated Then db.AddInParameter(dbcmd, "@PsDesc", DbType.String, SC_SendMailBadRow.PsDesc.Value)
            If SC_SendMailBadRow.LinkPath.Updated Then db.AddInParameter(dbcmd, "@LinkPath", DbType.String, SC_SendMailBadRow.LinkPath.Value)
            If SC_SendMailBadRow.Attachment.Updated Then db.AddInParameter(dbcmd, "@Attachment", DbType.String, SC_SendMailBadRow.Attachment.Value)
            If SC_SendMailBadRow.CreateDate.Updated Then db.AddInParameter(dbcmd, "@CreateDate", DbType.Date, IIf(IsDateTimeNull(SC_SendMailBadRow.CreateDate.Value), Convert.ToDateTime("1900/1/1"), SC_SendMailBadRow.CreateDate.Value))
            db.AddInParameter(dbcmd, "@PKArrivalDt", DbType.String, IIf(SC_SendMailBadRow.LoadFromDataRow, SC_SendMailBadRow.ArrivalDt.OldValue, SC_SendMailBadRow.ArrivalDt.Value))
            db.AddInParameter(dbcmd, "@PKMailSeqNo", DbType.Int32, IIf(SC_SendMailBadRow.LoadFromDataRow, SC_SendMailBadRow.MailSeqNo.OldValue, SC_SendMailBadRow.MailSeqNo.Value))

            Return db.ExecuteNonQuery(dbcmd)
        End Function

        Public Function Update(ByVal SC_SendMailBadRow As Row, ByVal tran As DbTransaction) As Integer
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()
            Dim strDot As String = String.Empty
        ...
        Public Function IsDataExists(ByVal SC_SendMailBadRow As Row) As Boolean
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()
            Dim intCount As Integer = 0

            strSQL.AppendLine("Select Count(*) Cnt From SC_SendMailBad")
            strSQL.AppendLine("Where ArrivalDt = @ArrivalDt")
            strSQL.AppendLine("And MailSeqNo = @MailSeqNo")
        ...
        Public Function IsDataExists(ByVal SC_SendMailBadRow As Row, ByVal tran As DbTransaction) As Boolean
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()
            Dim intCount As Integer = 0
        ...
        Public Function Insert(ByVal SC_SendMailBadRow As Row) As Integer
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()

            strSQL.AppendLine("Insert into SC_SendMailBad")
            strSQL.AppendLine("(")
            ...
            db.AddInParameter(dbcmd, "@ArrivalDt", DbType.String, SC_SendMailBadRow.ArrivalDt.Value)
            db.AddInParameter(dbcmd, "@MailSeqNo", DbType.Int32, SC_SendMailBadRow.MailSeqNo.Value)
            db.AddInParameter(dbcmd, "@SenderID", DbType.String, SC_SendMailBadRow.SenderID.Value)
            db.AddInParameter(dbcmd, "@MailAddress", DbType.String, SC_SendMailBadRow.MailAddress.Value)
            db.AddInParameter(dbcmd, "@MailHeader", DbType.String, SC_SendMailBadRow.MailHeader.Value)
            db.AddInParameter(dbcmd, "@GreetWord", DbType.String, SC_SendMailBadRow.GreetWord.Value)
            db.AddInParameter(dbcmd, "@PsDesc", DbType.String, SC_SendMailBadRow.PsDesc.Value)
            db.AddInParameter(dbcmd, "@LinkPath", DbType.String, SC_SendMailBadRow.LinkPath.Value)
            db.AddInParameter(dbcmd, "@Attachment", DbType.String, SC_SendMailBadRow.Attachment.Value)
            db.AddInParameter(dbcmd, "@CreateDate", DbType.Date, IIf(IsDateTimeNull(SC_SendMailBadRow.CreateDate.Value), Convert.ToDateTime("1900/1/1"), SC_SendMailBadRow.CreateDate.Value))

            Return db.ExecuteNonQuery(dbcmd)
        End Function

        Public Function Insert(ByVal SC_SendMailBadRow As Row, ByVal tran As DbTransaction) As Integer
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()
        ...
        Public Function Insert(ByVal SC_SendMailBadRow As Row(), ByVal tran As DbTransaction) As Boolean
            Dim db As Database = DatabaseFactory.CreateDatabase()
            Dim dbcmd As DbCommand
            Dim strSQL As StringBuilder = New StringBuilder()
            Dim intRowsAffected As Integer = 0
