...
#Region "畫面邏輯處理"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            '2017/05/08 - HR say 加班轉換方式要與前台顯示方式相同, EX初始帶入轉薪資。
            ddlSalaryOrAdjust.SelectedIndex = 1
    ...
    Protected Sub LoadData()
        ...
        '    Dim OverTimeSumArr As ArrayList = objOV11.GetOverTimeSum(UserProfile.ActCompID.ToString().Trim, txtOTEmpID.Text.Trim, ucOTStartDate.DateText.ToString())
        ...
        _dtPara = objOV11.Json2DataTable(objOV11.QueryColumn("Para", "OverTimePara", " AND CompID = '" + hidCompID.Value.ToString() + "'"))      '帶入參數設定檔
    ...
    Protected Sub txtOTEmpID_TextChanged(sender As Object, e As System.EventArgs) Handles txtOTEmpID.TextChanged
        ...
        If txtOTEmpID.Text.Trim <> "" Then
            Dim rtnTable As DataTable = objOV11.GetEmpName(hidCompID.Value, txtOTEmpID.Text.Trim)
        ...
        If returnValue <> "" Then
            ...
            Select Case aryData(0)
                Case "ucQueryEmp"
                    ...
                    _dtPara = objOV11.Json2DataTable(objOV11.QueryColumn("Para", "OverTimePara", " AND CompID = '" + aryValue(3).Trim + "'"))
    ...
    Protected Sub LoadOTEmpData()
        ...
        Using dt As DataTable = objOV11.GetEmpData(hidCompID.Value.ToString().Trim, txtOTEmpID.Text.Trim)
    ...
    Protected Sub rblOTPerson_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles rblOTPerson.SelectedIndexChanged
        ...
        Select Case rblOTPerson.SelectedIndex
            Case 0      '單筆加班人
                ...
                '附件也需要清掉
                objOV11.DeleteAttach(_AttachID)
...
#Region "暫存、送簽、刪除、取消"
    ...
    Protected Sub btnSubmit_Click(sender As Object, e As System.EventArgs) Handles btnSubmit.Click
        ...
        If gvMain.Rows.Count > 0 Then
            ...
            sb.Append(" VALUES('AP', 'OT', 'OV1101', '" + UserProfile.ActUserID + "', '" + hidGuidID.Value + "', '" + Newtonsoft.Json.JsonConvert.SerializeObject(DataList) + "', GETDATE(), '30')")
            Bsp.DB.ExecuteNonQuery(CommandType.Text, sb.ToString, "AattendantDB")
        End If

        If chkCnt = 0 Then
            If _FormNo = "" Then
                _FormNo = objOV11.QueryFormNO("AdvanceFormSeq", UserProfile.ActCompID.Trim, UserProfile.UserID.Trim)
            ElseIf rblOTPerson.SelectedValue = "0" Then
                _FormNo = objOV11.QueryFormNO("AdvanceFormSeq", UserProfile.ActCompID.Trim, UserProfile.UserID.Trim)
            End If


            Dim attach = objOV11.QueryAttach(_AttachID, hidCompID.Value.ToString().Trim, txtOTEmpID.Text.Trim)
...
#Region "執行暫存加班單動作"
    ...
    Private Function SaveTempData() As Boolean
        ...
        OTSeq = objOV11.QuerySeq("OverTimeDeclaration", hidCompID.Value.ToString().Trim, txtOTEmpID.Text.Trim, ucOTStartDate.DateText)
        If ucOTStartDate.DateText = ucOTEndDate.DateText Then   '不跨日送簽
            Dim strHo As String = If(objOV11.CheckHolidayOrNot(ucOTStartDate.DateText), "1", "0")
        ...
        Else    '跨日暫存
            ...
            ' 判斷迄日是否需要扣除不足的用餐時間
            ' </summary>
            Dim mealOver As String = objOV11.MealJudge(cntStart, Convert.ToDouble(txtMealTime.Text.Trim))

            Dim strHo1 As String = If(objOV11.CheckHolidayOrNot(ucOTStartDate.DateText), "1", "0")
            Dim strHo2 As String = If(objOV11.CheckHolidayOrNot(ucOTEndDate.DateText), "1", "0")
            ...
            For i = 0 To (crossDayArray.Split(",").Length - 1)    'crossDayArray->方便拆單用
                ...
                Else
                    OTSeqs = objOV11.QuerySeq("OverTimeDeclaration", hidCompID.Value.ToString().Trim, txtOTEmpID.Text.Trim, crossDayArray.Split(",")(1))
...
#Region "清除送簽快取"
    ...
    Private Function ClearCacheData() As Boolean
        Dim sb As New StringBuilder
        sb.Append("DELETE * FROM CacheData WHERE 1=1 AND UserID = " + Bsp.Utility.Quote(UserProfile.ActUserID) + " AND Platform = 'AP' AND SystemID = 'OT' AND TxnName = 'OV1101'")
        Try
        ...
        Select Case chkCopyAtt.Checked
            Case True
                If ViewState("attach").ToString().IndexOf("test") < 0 Then
                    ...
                    objOV11.InsertAttach(_AttachID, ViewState("attach").ToString())
...
#Region "多筆送簽檢核"
    ...
    Private Function funCheckMultiSubmit() As Boolean
        ...
        '讀取參數檔
        Dim _dtPara As DataTable = objOV11.Json2DataTable(objOV11.QueryColumn("Para", "OverTimePara", " AND CompID = " + Bsp.Utility.Quote(hidCompID.Value.ToString().Trim)))
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                For i = 0 To (dt.Rows.Count - 1)
                    ...
                    Dim strCheckOverTimeIsOver As String = objOV11.GetCheckOverTimeIsOver(dt, dayNLimit, dayHLimit, "OverTimeDeclaration")
                    ...
                    '檢查每個月的加班時數是否超出上限
                    Dim resultMsg As String = objOV11.GetMulitTotal(dt, Convert.ToDouble(_dtPara.Rows(0)("MonthLimitHour").ToString()), "OverTimeDeclaration")
                    ...
                    '檢查連續上班是否超過限制
                    Dim strGetCheckOTLimitDay As String = objOV11.GetCheckOTLimitDay(dt, _dtPara.Rows(0)("OTLimitDay").ToString(), "OverTimeDeclaration")
...
#End Region
    ...
    Private Function funCheckData() As Boolean
        ...
        If ChkData Then
            ...
            If objOV11.CheckHolidayOrNot(ucOTStartDate.DateText) Then  '如果加班起日是假日
                If ucOTStartDate.DateText <> ucOTEndDate.DateText Then
                    ...
                    ElseIf (Not OVBusinessCommon.IsNationalHoliday(ucOTStartDate.DateText)) Then    '如果加班起日是非周末之一般假日
                        ...
                        Else
                            If Not objOV11.CheckNHolidayOTOrNot(hidCompID.Value.ToString().Trim, txtOTEmpID.Text.Trim, ucOTStartDate.DateText, ucOTEndDate.DateText, "", "Add", ChkMsg) Then
                                ErrMsg += ChkMsg
                            End If
                        End If
                    End If
            ...
            '    '檢查事先申請是否連續加班
            '    If objOV11.CheckHolidayOrNot(a.ToString("yyyy/MM/dd")) Then
            '        Using dt As DataTable = objOV11.QueryData("*", "OverTimeAdvance", " AND OTStartDate= " + Bsp.Utility.Quote(a.ToString("yyyy/MM/dd")) + " AND OTCompID=" + Bsp.Utility.Quote(hidCompID.Value.ToString()) + " AND OTEmpID =" + Bsp.Utility.Quote(txtOTEmpID.Text.Trim) + " AND OTStatus in ('1','2','3') AND OTStartDate <> ( SELECT Code FROM AT_CodeMap WHERE Code = OTStartDate AND NotShowFlag = '0' AND TabName = 'NationalHolidayDefine' AND FldName ='HolidayDate')")
            '            If dt.Rows.Count > 0 Then
            '                'Bsp.Utility.ShowMessage(Me, "不能假日連續加班")
            '                'Return False
            '                '20170314-HR不擋僅通知-讓檢核能通過
            ...
            If Month(Convert.ToDateTime(ucOTStartDate.DateText)) = Month(Convert.ToDateTime(ucOTEndDate.DateText)) Then     '無跨月
                If Not objOV11.checkMonthTime("OverTimeDeclaration", hidCompID.Value.ToString(), txtOTEmpID.Text, ucOTStartDate.DateText, ucOTEndDate.DateText, Convert.ToDouble(_dtPara.Rows(0)("MonthLimitHour").ToString()), cntTotal, Convert.ToDouble(txtMealTime.Text.Trim), _
            ...
            Else    '有跨月
                ...
                '先算加班開始日期部分
                If Not objOV11.checkMonthTime("OverTimeDeclaration", hidCompID.Value.ToString(), txtOTEmpID.Text, ucOTStartDate.DateText, ucOTStartDate.DateText, Convert.ToDouble(_dtPara.Rows(0)("MonthLimitHour").ToString()), cntStart, mealOver.Split(","c)(1), _
                        cntStart, 0, "") Then
                    '20170314-HR不擋僅通知-讓檢核能通過
                    'Bsp.Utility.ShowMessage(Me, "每月上限加班申報時數為" + _dtPara.Rows(0)("MonthLimitHour") + "小時")
                    'Return False
                ...
                '再算加班結束日期部分
                If Not objOV11.checkMonthTime("OverTimeDeclaration", hidCompID.Value.ToString(), txtOTEmpID.Text, ucOTEndDate.DateText, ucOTEndDate.DateText, Convert.ToDouble(_dtPara.Rows(0)("MonthLimitHour").ToString()), cntEnd, mealOver.Split(","c)(3), _
                        cntEnd, 0, "") Then
                    '20170314-HR不擋僅通知-讓檢核能通過
                    'Bsp.Utility.ShowMessage(Me, "每月上限加班申報時數為" + _dtPara.Rows(0)("MonthLimitHour") + "小時")
                    'Return False
            ...
            '檢核時間重疊(OverTime_BK)
            Using dt As DataTable = objOV11.CheckOverTimeBK(ucOTStartDate.DateText, ucOTEndDate.DateText, txtOTEmpID.Text.Trim)
                If (dt.Rows.Count > 0) Then
            ...
            Using dt As DataTable = objOV11.CheckNaturalDisasterByCity(ucOTStartDate.DateText, ucOTEndDate.DateText, _WorkSiteID, hidCompID.Value.ToString())
                If (dt.Rows.Count > 0) Then
                    For i = 0 To (dt.Rows.Count - 1)
                        '起迄時間都有重疊
                        If ((dt.Rows(i).Item("BeginTime").ToString() = ucOTStartTime.ucDefaultSelectedHH + ucOTStartTime.ucDefaultSelectedMM) And (dt.Rows(i).Item("EndTime").ToString() = ucOTEndTime.ucDefaultSelectedHH + ucOTEndTime.ucDefaultSelectedMM)) Then
            ...
            '檢核時間重疊(NaturalDisasterByCity)
            Using dt As DataTable = objOV11.CheckNaturalDisasterByEmp(ucOTStartDate.DateText, ucOTEndDate.DateText, txtOTEmpID.Text.Trim, hidCompID.Value.ToString())
                If (dt.Rows.Count > 0) Then
                    For i = 0 To (dt.Rows.Count - 1)
                        '起迄時間都有重疊
                        If ((dt.Rows(i).Item("BeginTime").ToString() = ucOTStartTime.ucDefaultSelectedHH + ucOTStartTime.ucDefaultSelectedMM) And (dt.Rows(i).Item("EndTime").ToString() = ucOTEndTime.ucDefaultSelectedHH + ucOTEndTime.ucDefaultSelectedMM)) Then
            ...
            '檢核時間重疊(事後申報)
            Using dt As DataTable = objOV11.CheckOverTimeDeclaration(ucOTStartDate.DateText, ucOTEndDate.DateText, txtOTEmpID.Text.Trim, hidCompID.Value.ToString())
                If (dt.Rows.Count > 0) Then
                    If ucOTStartDate.DateText = ucOTEndDate.DateText Then
                        For i = 0 To (dt.Rows.Count - 1)
                            '起迄時間都有重疊  
            ...
            '檢核時間重疊(預先申請)
            Using dt As DataTable = objOV11.CheckOverTimeAdvance(ucOTStartDate.DateText, ucOTEndDate.DateText, txtOTEmpID.Text.Trim, hidCompID.Value.ToString())
                If (dt.Rows.Count > 0) Then
                    If ucOTStartDate.DateText = ucOTEndDate.DateText Then
                        For i = 0 To (dt.Rows.Count - 1)
                            '起迄時間都有重疊 
                            If (ucOTStartDate.DateText = ucOTEndDate.DateText) Then
                                ...
                                End If
                            Else
                                Dim strTxnID As String = objOV11.QueryColumn("OTEndTime", "OverTimeAdvance", " AND OTTxnID='" + dt.Rows(i).Item("OTTxnID").ToString() + "' AND OTSeqNo='2'")
                ...
                If (txtOTEmpID.Text <> "") Then
                    ...
                    If bOTTimeStart And bOTTimeEnd Then
                        ...
                        If (Integer.TryParse(sOTTimeStart, iOTTimeStart) AndAlso (Integer.TryParse(sOTTimeEnd, iOTTimeEnd))) Then
                            ...
                            Dim bPeriodCount As Boolean = objOV11.PeriodCount("OverTimeDeclaration", txtOTEmpID.Text.Trim, cntTotal, 0, StartDateArr, EndDateArr, iMealTime, smealFlag, "", returnPeriodCount)
                ...
                If txtOTEmpID.Text <> "" Then
                    ...
                    If bOTTimeStart AndAlso bOTTimeEnd Then
                        ...
                        If (Integer.TryParse(sOTTimeStart, iOTTimeStart) And (Integer.TryParse(sOTTimeEnd, iOTTimeEnd))) Then
                            ...
                            Dim bPeriodCount As Boolean = objOV11.PeriodCount("OverTimeDeclaration", txtOTEmpID.Text.Trim, cntStart, cntEnd, StartDateArr, EndDateArr, iMealTime, smealFlag, "", returnPeriodCount)
    ...
    Protected Sub MealFlag_CheckedChanged(sender As Object, e As System.EventArgs) Handles MealFlag.CheckedChanged
        ...
        Else
            ...
            If cntTotal > 120 Then
                '加班超過兩小時需扣用餐時間60分鐘
                txtMealTime.Enabled = True
                Dim strHo = objOV11.CheckHolidayOrNot(ucOTStartDate.DateText)
                If Not String.IsNullOrEmpty(strHo) Then
                    If strHo = "0" Then
