...
#Region "功能群組"
    ...
    Private Sub Update()
        ...
        '如果附件的key有資料
        Dim objOV12 = New OV1_2
        If labOTAttachment.Value.ToString.Trim.Count > 0 Then
            labOTAttachment.Value = labOTAttachment.Value.ToString
        Else
            labOTAttachment.Value = objOV12.QueryAttach(_AttachID, ViewState("OTCompID"), ViewState("OTEmpID"))
        End If
            ...
            If OTStartDate.Equals(OTEndDate) Then '改成無跨日
                ...
                If ViewState("Type").Equals("bef") Then
                    ...
                    If labOverTimeDate.Value.ToString.Split("~")(0).Equals(labOverTimeDate.Value.ToString.Split("~")(1)) Then
                        Dim Flag As Boolean = OV_3.UpDateFor1002Single(OV_3)
                        showMagAndGoBackFromflag(Flag)
                    Else ' 有跨->無跨 upDate seqNO 1 remove seqNO 2
                        Dim Flag As Boolean = OV_3.DeleteFor1002(OV_3, "2")
                        If Flag Then
                            Flag = OV_3.UpDateFor1002Single(OV_3)
                        End If
                        showMagAndGoBackFromflag(Flag)
                    End If
                ElseIf (ViewState("Type").Equals("after")) Then
                    ...
                    '20170308 用來紀錄是否要跟改ToOverTimeFlag 在 OV_3.UpDateFor1002Single(OV_3) 及OV_3.InsertFor1002 用到
                    ...
                    If OV_3.isToOverTime() And OV_3.isSalaryPaid Then
                        ...
                        If labOverTimeDate.Value.ToString.Split("~")(0).Equals(labOverTimeDate.Value.ToString.Split("~")(1)) Then
                            Flag = OV_3.UpDateFor1002Single(OV_3)
                        Else '有跨->無跨 upDate seqNO 1 remove seqNO 2
                            Flag = OV_3.DeleteFor1002(OV_3, "2")
                            If Flag Then
                                Flag = OV_3.UpDateFor1002Single(OV_3)
                            End If
                        End If
            ...
            Else '改成有跨日
                ...
                If ViewState("Type").Equals("bef") Then
                    ...
                    If DataTable.Rows.Count > 0 Then
                        Dim oldOVA As OV1_OVA = setProperTyForOV1_OVA(DataTable)
                        Dim Flag As Boolean = OV_3.DeleteFor1002(OV_3, "")
                        If Flag Then
                            Flag = OV_3.InsertFor1002OVA(OV_3, oldOVA)
                        End If
                        showMagAndGoBackFromflag(Flag)
                    End If
                ElseIf (ViewState("Type").Equals("after")) Then
                    ...
                    If OV_3.isToOverTime() And OV_3.isSalaryPaid Then
                        ...
                        '不管有無跨日我都 先查 舊資料 抓取他到Server 然刪除舊資料 新增兩筆
                        If oldDataTable.Rows.Count > 0 Then
                            Dim oldOVD As OV1_OVD = setProperTyForOV1_OVD(oldDataTable)
                            Dim Flag As Boolean = OV_3.DeleteFor1002(OV_3, "")
                            If Flag Then
                                Flag = OV_3.InsertFor1002OVD(OV_3, oldOVD)
                            End If
...
#Region "載入頁面"
    ...
    Public Sub LoanData(ByVal dt As DataTable, ByVal dt2 As DataTable)
        ...
        Dim strAttachAdminBaseURL As String = ConfigurationManager.AppSettings("AattendantWebPath") + "HandlerForOverTime/SSORecvModeForOverTime.aspx?UserID=" + UserProfile.UserID.Trim + "&SystemID=OT&TxnID=OV1202&ReturnUrl=%2FUtil%2FAttachAdmin.aspx?AttachDB={0}%26AttachID={1}%26AttachFileMaxQty={2}%26AttachFileMaxKB={3}%26AttachFileTotKB={4}%26AttachFileExtList={5}"
...
#Region "資料檢核"
    ...
    Private Function chackEndToUPdata() As String
        ...
        Dim objOV12 As OV1_2 = New OV1_2
        ...
        Using dt As DataTable = objOV12.GetEmpData(ViewState("OTCompID").Trim, EmpID)
            If dt.Rows.Count > 0 Then
        ...
        '檢核時間重疊(OverTime_BK)
        Using dt As DataTable = objOV12.CheckOverTimeBK(OTStartDate, OTEndTime, EmpID)
            If (dt.Rows.Count > 0) Then
        ...
        Using dt As DataTable = objOV12.CheckNaturalDisasterByCity(OTStartDate, OTEndDate, WorkSiteID, UserProfile.SelectCompRoleID)
            If (dt.Rows.Count > 0) Then
        ...
        '檢核時間重疊(NaturalDisasterByCity)
        Using dt As DataTable = objOV12.CheckNaturalDisasterByEmp(OTStartDate, OTEndDate, EmpID, UserProfile.SelectCompRoleID)
            If (dt.Rows.Count > 0) Then
                For i = 0 To (dt.Rows.Count - 1)
                    '起迄時間都有重疊
                    If ((dt.Rows(i).Item("BeginTime").ToString() = OTStartTime) And (dt.Rows(i).Item("EndTime").ToString() = OTEndTime)) Then
...
#End Region
    ...
    ''' <summary>
    ''' 傳入事先申請DataTable 轉換成OV1_OVA物件
    ''' </summary>
    ''' <param name="DataTable"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function setProperTyForOV1_OVA(ByVal DataTable As DataTable) As OV1_OVA
        Dim ova As OV1_OVA = New OV1_OVA
    ...
    ''' <summary>
    '''  傳入事後申報DataTable 轉換成OV1_OVD物件
    ''' </summary>
    ''' <param name="DataTable"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function setProperTyForOV1_OVD(ByVal DataTable As DataTable) As OV1_OVD
        Dim ovd As OV1_OVD = New OV1_OVD
    ...
    Private Sub controlTimeText()
        ...
        labAdjustInvalidDate.Text = OV_3.getOV1002labAdjustInvalidDate(OTCompID)
        ...
        '****時段計算**************************************************************************************
        If (labOTStatus.Text.Equals("核准") Or (labOTStatus.Text.Equals("送簽"))) Then
            Dim dt As DataTable = OV_3.getOV1002ForTimeDataTable(ViewState("OTCompID").ToString(), ViewState("OTEmpID").ToString, StartDate, EndDate, StartTime, EndTime, "", ViewState("OTTxnID").ToString, meatFlag, meatTime)
            If dt.Rows.Count > 0 Then
                Dim item As DataRow = dt.Rows(0)
...
#Region "附件專區"
    ...
    Protected Sub getAttachName()
        ...
        Using dt As DataTable = objOV12.QueryData("ISNULL(FileName,'') AS FileName", "AttachInfo", "AND FileSize>0 AND AttachID=" + Bsp.Utility.Quote(_AttachID))
            If dt.Rows.Count > 0 Then
        ...
        Select Case chkCopyAtt.Checked
            Case True
                If ViewState("attach").ToString().IndexOf("test") < 0 Then
                    ...
                    objOV12.InsertAttach(_AttachID, ViewState("attach").ToString())
                Else
            ...
            Case False
                If ViewState("attach").ToString().IndexOf("test") < 0 Then
                    '附件也需要清掉
                    objOV12.DeleteAttach(_AttachID)
                    strAttachID = _AttachID
        ...
        If Not String.IsNullOrEmpty(sRankID) AndAlso Not String.IsNullOrEmpty(startDate) AndAlso Not String.IsNullOrEmpty(endDate) Then
            If IsNumeric(sRankID) = True Then
                ...
                ElseIf dRankID < 19 Then
                    ...
                    Dim dateStartIsHoliday As Boolean = objOV12.CheckHolidayOrNot(startDate)
                    Dim dateEndIsHoliday As Boolean = objOV12.CheckHolidayOrNot(endDate)
