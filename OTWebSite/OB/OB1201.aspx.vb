...
#Region "2. 功能鍵處理邏輯"
    ...
    Private Function SaveData() As Boolean
        ...
        '儲存資料
        Try
            If AddOTSUsrSetting(bePOSPUser) Then
    ...
    Private Sub ClearData()
        ...
        hidchkOrganFlow.Value = String.Empty
        ddlBusinessType.SelectedIndex = 0
        hidSelectedBSValue.Value = String.Empty
...
#Region "6. 畫面檢核與確認"
    ...
    Private Function funCheckData() As Boolean
        ...
        ElseIf ddlCompID.SelectedValue = hidCompID.Value Then
            bePOSPUser.CompID.Value = ddlCompID.SelectedValue
        Else
            bePOSPUser.CompID.Value = hidCompID.Value
        End If

        bePOSPUser.EmpID.Value = txtEmpID.Text.Trim.ToString()

        '檢查是否有輸入
        If Not String.IsNullOrEmpty(txtEmpID.Text.Trim) AndAlso Not String.IsNullOrEmpty(lblEmpName.Text.Trim) Then
            '檢查是否有選擇行政組織跟功能組織
...
#Region "7. private Method"
    ...
    ''' 新增資料至資料庫的處理動作
    ''' </summary>
    ''' <param name="bePOSPUser">要新增的資料</param>
    ''' <returns>Boolean</returns>
    ''' <remarks></remarks>
    Private Function AddOTSUsrSetting(ByVal bePOSPUser As bePOSPUser.Row) As Boolean
        Dim bsPOSPUser As New bePOSPUser.Service()
        ...
        Using cn As DbConnection = Bsp.DB.getConnection("AattendantDB")
            ...
            Try
                If bsPOSPUser.Insert(bePOSPUser, tran) = 0 Then Return False
                tran.Commit()
            Catch ex As Exception
                tran.Rollback()
        ...
        Try
            Select Case ddlBusinessType.SelectedValue
                Case ""     '請選擇
                    ...
                    For Each item As ListItem In ddlBusinessType.Items
                        If Not String.IsNullOrEmpty(item.Value) Then ArrBusinessType.Add(item.Value)
                    Next
            ...
            If dt.Rows.Count > 0 Then
                For Each item As DataRow In dt.Rows
                    ...
                    Else
                        Dim BS As Integer = 0
                        If String.IsNullOrEmpty(ddlBusinessType.SelectedValue) Then
            ...
            '拆字串存進ArrayList
            If Not String.IsNullOrEmpty(hidchkOrganFlow.Value.Trim()) Then
...
#Region "部門選單-行政組織"
    ...
    Private Sub subLoadTreeOrgan(ByVal objTV As TreeView, ByVal strCompID As String)
        ...
        Using dt As DataTable = objSPUsr.GetOrgMenu(strCompID)
            ...
            If dt.Rows.Count > 0 Then
                For Each item As DataRow In dt.Rows
                    If item("SortNode").ToString().Equals("1") Then
                        Dim node As New TreeNode
                ...
                For Each item As DataRow In dt.Rows
                    If item("SortNode").ToString().Equals("2") Then
                        Dim node As New TreeNode
                ...
                For Each item As DataRow In dt.Rows
                    If item("SortNode").ToString().Equals("3") Then
                        Dim node As New TreeNode
        ...
        Try
            ...
            Select Case hidSelectedBSValue.Value.ToString
                ...
                Case Else   '其他業務別
                    ...
                    '拆字串存進ArrayList
                    If Not String.IsNullOrEmpty(OrganFlowStr.Trim) Then
                        'Note: the ","c is a character literal, where as "," is a string literal.
                        ArrFlow.AddRange(OrganFlowStr.Split(","c))
...
#Region "隱藏選單"
    ...
    ''' 長出隱藏選單
    ''' </summary>
    ''' <param name="objTV">暫存功能組織的隱藏treeView元件</param>
    ''' <param name="strCompID">公司別</param>
    ''' <remarks></remarks>
