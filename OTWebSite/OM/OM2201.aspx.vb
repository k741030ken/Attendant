...
Partial Class OM_OM2201
    ...
    Protected Overrides Sub BaseOnPageTransfer(ByVal ti As TransferInfo) '返回查詢頁的動作(為了撈回查詢的資料，再做一次查詢 => 利用暫存)
        If Not IsPostBack() Then
            ...
            hidCompID.Value = aryValue(0) '公司代碼

            txtOrganID.Text = ht(txtOrganID.ID).ToString() & "-" & ht(txtOrganName.ID).ToString()  '部門代碼-名稱
            hidOrganID.Value = ht(txtOrganID.ID).ToString().Trim  '部門代碼

            aryValue = Split(ht("SelectedOrganType").ToString(), "-") '組織類型
    ...
    Private Sub GetSelectData()
        ...
        Try
            ...
            If ddlOrganReason.SelectedValue <> "1" Then
                ...
                Using dt As DataTable = objOM.OM2200OrganizationLog(hidCompID.Value, "", hidOrganType.Value, ucValidDateB.DateText, Old, hidOrganID.Value, "")
...
#End Region
    ...
    Private Function SaveData() As Boolean
        ...
        Try
            Return objOM1.InsertOLAddition(beOrganizationLog)
        Catch ex As Exception
