...
Partial Class Component_PageWorkType
    Inherits PageBase
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack() Then
            ...
            If Not Session(Request("ControlSessionID")) Is Nothing Then
                ...
                ViewState.Item("DefaultWorkType") = aryParam(2)
                ...
                If ViewState.Item("DefaultWorkType") <> "" Then
                    '自訂內容(已選)
                    LoadRightData()
                End If
        ...
        If Not Page.ClientScript.IsClientScriptBlockRegistered("UserControl") Then
            ...
            Me.Page.ClientScript.RegisterClientScriptBlock(cstype, "PageWorkType", script)
        End If
    ...
    Public Sub LoadLeftData()
        ...
        Try
            ...
            If txtQueryString.Text.Trim().Length > 0 Then
                ...
                'strWhereC = " And [" & ddlField.SelectedValue & "] like '%" & txtQueryString.Text.Trim() & "%'"
            End If

            If ViewState.Item("DefaultWorkType").ToString() <> "" Then
                strWhere += "And WorkTypeID not in ( " & ViewState.Item("DefaultWorkType").ToString() & ")"

                '2016/04/29 SPHBKC資料已併入WorkTypeID中
                'If ViewState.Item("QueryCompID").ToString = "SPHBKC" Then
                '    strWhereC += "And Code not in ( " & ViewState.Item("DefaultWorkType").ToString() & ")"
                'Else
                '    strWhere += "And WorkTypeID not in ( " & ViewState.Item("DefaultWorkType").ToString() & ")"
                'End If
            End If
            ...
            If ViewState.Item("IsWait") = True Then
                If ViewState.Item("QueryOrganID").ToString() <> "" Then
                    strWhere += "And WorkTypeID in (Select WorkTypeID From OrgWorkTypeWait Where OrganReason = '1' And WaitStatus = '0' And OrganType In ('1', '3')"
            ...
            Else
                If ViewState.Item("QueryOrganID").ToString() <> "" Then
                    strWhere += "And WorkTypeID in (Select WorkTypeID From OrgWorkType Where CompID = " & Bsp.Utility.Quote(ViewState.Item("QueryCompID").ToString) & " And OrganID=" & Bsp.Utility.Quote(ViewState.Item("QueryOrganID").ToString) & ")"
                End If
            End If

            '2016/04/29 SPHBKC資料已併入WorkTypeID中
            'If ViewState.Item("QueryCompID").ToString = "SPHBKC" Then
            '    Using dt As DataTable = objUC.GetCWorkTypeByUC("", "Code as WorkTypeID, Code + '-' + CodeName ", strWhereC)
            '        With lstLeft
            '            .DataSource = dt
            ...
            '    End Using
            'Else
            Using dt As DataTable = objUC.GetWorkTypeByUC("", "WorkTypeID, WorkTypeID + '-' + Remark ", strWhere)
                With lstLeft
                    .DataSource = dt
                    .DataValueField = dt.Columns(0).ColumnName
                    .DataTextField = dt.Columns(1).ColumnName
        ...
        Catch ex As Exception
            Bsp.Utility.ShowMessage(Me, "PageWorkType.LoadLeftData", ex)
            Return
        End Try
    ...
    Public Sub LoadRightData()
        ...
        Try
            ...
            '輸入多筆的工作性質，因第一筆為主要工作性質，為了要lstRight按照此順序顯示，只能BY每一筆GetWorkType再寫入lstRight
            Dim aryWorkType() As String = ViewState.Item("DefaultWorkType").ToString().Replace("'", "").Split(",")
            Dim strCompID As String = Bsp.Utility.Quote(ViewState.Item("QueryCompID").ToString)

            For intLoop As Integer = 0 To aryWorkType.GetUpperBound(0)
                If aryWorkType(intLoop) <> "" Then

                    '2016/04/29 SPHBKC資料已併入WorkTypeID中
                    'If ViewState.Item("QueryCompID").ToString = "SPHBKC" Then
                    '    strWhere = "And Code ='" & aryWorkType(intLoop) & "'"
                    '    Using dt As DataTable = objUC.GetCWorkTypeByUC(aryWorkType(intLoop), "Code as WorkTypeID, Code + '-' + CodeName ", strWhere)
                    ...
                    '        Else
                    '            Dim myitem As New ListItem
                    '            myitem.Value = aryWorkType(intLoop)
                    '            myitem.Text = aryWorkType(intLoop) + "-"
                    '            lstRight.Items.Add(myitem)
                    '            If strValue <> "" Then strValue &= ","
                    '            If strText <> "" Then strText &= ","
                    '            strValue &= aryWorkType(intLoop)
                    '            strText &= ""
                    '        End If

                    '    End Using
                    'Else
                    strWhere = "And WorkTypeID ='" & aryWorkType(intLoop) & "'"
                    strWhere = strWhere & " And CompID =" & strCompID & ""
                    Using dt As DataTable = objUC.GetWorkTypeByUC(aryWorkType(intLoop), "WorkTypeID, WorkTypeID + '-' + Remark ", strWhere)
        ...
        Catch ex As Exception
            Bsp.Utility.ShowMessage(Me.Page, "PageWorkType.LoadRightData", ex)
            Return
        End Try
    ...
    Private Sub RebuildList()
        Try
            ...
            End If
        Catch ex As Exception
            Bsp.Utility.ShowMessage(Me.Page, Bsp.Utility.getInnerException("PageWorkType.RebuildList", ex))
            Return
        End Try
    End Sub
