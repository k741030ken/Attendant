    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack() Then
            ...
            If Request.Form.Count > 0 Then
                ...
                If ht.ContainsKey("SSOTime") AndAlso ht.ContainsKey("UserID") AndAlso ht.ContainsKey("MD5") AndAlso ht.ContainsKey("SSOID") AndAlso ht("SSOTime").ToString() <> "" Then
                    ...
                    If ht("MD5").ToString() = strAfterHash Then
                        If ht.ContainsKey("TimeStamp") AndAlso ht("TimeStamp").ToString() <> "" Then
                            '20150520 wei add 模式二
                            Dim Chk_TimeStamp As String = ""
                            Dim vChkURL As String = ConfigurationManager.AppSettings("ePortal")
                            Dim vChkURLParam As String = ""
                            vChkURL = vChkURL & "?ChkUserID=" & ht("UserID").ToString() & "&SSOID=" & ht("SSOID").ToString() & "&MD5=" + ht("MD5").ToString() & "&TimeStamp=" & ht("TimeStamp").ToString() '模式二驗證組網址
                            vChkURLParam = "ChkUserID=" & ht("UserID").ToString() & "&SSOID=" & ht("SSOID").ToString() & "&MD5=" + ht("MD5").ToString() & "&TimeStamp=" & ht("TimeStamp").ToString() ''式二驗證組網址

                            Dim web_request As HttpWebRequest = WebRequest.Create(vChkURL) ''模式二驗證
                            Dim mResponse As HttpWebResponse = web_request.GetResponse()
                            Dim sr As New StreamReader(mResponse.GetResponseStream, Encoding.GetEncoding("utf-8")) ' 可設定用什麼編碼來擷取
                            ...
                            If strContent.ToString().Substring(0, 1) = "Y" Then
                                ...
                                Login(strUserID)
                            Else
                                strPage = Bsp.Utility.getMessagePageURL("Login_Load_2", Bsp.Enums.MessageType.Information, "不正確登入此系统(Timestamp驗證失敗)" + strContent.ToString()  + "，請由入口網站重新登入5！" + vChkURL , "關閉視窗", "")
                                FormsAuthentication.SetAuthCookie("ClosePage", False)
                                Server.Transfer(Me.ResolveUrl(strPage))
                            End If
                        Else
                ...
                End If
            Else
                strPage = Bsp.Utility.getMessagePageURL("Login_Load_2", Bsp.Enums.MessageType.Information, "不正確登入此系统，請由入口網站重新登入3！", "關閉視窗", "")
                FormsAuthentication.SetAuthCookie("ClosePage", False)
                Server.Transfer(Me.ResolveUrl(strPage))
            End If
        Else
