...
public class FlowUtility
{
    ...
    /// <param name="sOTMode">A: 事先 D:事後</param>
    /// <param name="sOTEmpID">加班人</param>
    ...
    public static void InsertHROverTimeLogCommand(
        string flowCaseID, string flowLogBatNo, string flowLogID,
        string sOTMode, string sOTEmpID, string sEmpOrganID, string sEmpFlowOrganID, string sApplyID,
    ...
    {
        var CommandKey = "InsertHROverTimeLog_" + count + "_";
        var dateNow = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff");
        var meassage = "";
        var retrunTable = new DataTable("RetrunTable");
        if (QueryHROverTimeLog(flowCaseID, sOTMode, out retrunTable, out meassage) &&
        ...
        sb.AppendStatement(" INSERT INTO HROverTimeLog ");
        sb.Append(" ( ");
        sb.Append(" FlowCaseID, Seq ");
        sb.Append(" , FlowLogBatNo, FlowLogID ");
        sb.Append(" , OTMode, OTEmpID, EmpOrganID, EmpFlowOrganID, ApplyID ");
        ...
        sb.Append(" , ").AppendParameter(CommandKey + "OTMode", sOTMode); //A: 事先 D:事後
        sb.Append(" , ").AppendParameter(CommandKey + "OTEmpID", sOTEmpID); //加班人
    ...
    /// <returns></returns>
    public static bool QueryHRFlowEngineDatas_Now(string compID, string flowCaseID, string otModel, out DataRow thisRow, out string message)
     {
        var isSuccess = false;
        var systemID = "OT";
        message = "";        
    ...
    /// </summary>
    /// <param name="compID">公司</param>
    /// <param name="sOrganID">加班人行政線organID</param>
    /// <param name="sFlowOrganID">加班人功能線organID</param>
    /// <param name="empID">加班人</param>
    /// <param name="applyID">申請人</param>
    /// <param name="queryDate">加班日期</param>
    /// <param name="flowCodeFlag">0: 事前; 1: 事後</param>
    /// <param name="empData">回傳: 加班人資訊</param>
    ...
    public static bool QueryFlowDataAndToUserData_First(string compID, string sOrganID, string sFlowOrganID, string empID, string applyID, string queryDate, string flowCodeFlag,
        out Dictionary<string, string> empData, out Dictionary<string, string> toUserData, out string flowCode, out string flowSN, out bool nextIsLastFlow, out string meassge)
    {
        ...
        try
        {
                ...
                out flowOrganID, out flowDeptID, out flowUpOrganID, out flowBoss, out flowBossCompID, out businessType, out empFlowRemark))
            {
                ...
                if (QueryOTFlowData(compID, empID, flowCodeFlag, deptID, organID, businessType, rankID, titleID, empFlowRemark, positionID, workTypeID, out flowCode, out flowSN, out meassge))
                {
    ...
    /// <param name="meassge">回傳: 訊息</param>
    /// <returns>bool</returns>
    public static bool QueryFlowDataAndToUserData(string compID, string applyID, string queryDate, string flowCaseID, string flowCodeFlag,
        out Dictionary<string, string> toUserData, out string flowCode, out string flowSN, out string signLineDefine, out bool isLastFlow, out bool nextIsLastFlow, out string meassge)
    {
        var isSuccess = false;
        meassge = "";
        toUserData = new Dictionary<string, string>();
        ...
        try
        {
            ...
            if (QueryToUserData(compID, applyID, queryDate, flowCaseID, otModle, out toUserData, out flowCode, out flowSN, out signLineDefine, out isLastFlow, out nextIsLastFlow, out meassge))
            {
                isSuccess = true;
            }
        }
    ...
    /// </summary>
    /// <param name="compID">公司</param>
    /// <param name="otEmpID">加班者</param>
    /// <param name="applyID">申請者</param>
    ...
    /// <returns>bool</returns>
    private static bool QueryToUserDataFirst(string compID, string otEmpID, string organID, string flowOrganID, string applyID, string queryDate, string flowCode, string flowSN,
        out Dictionary<string, string> toUserData, out bool nextIsLastFlow, out string message)
    {
        ...
        try
        {
            if (QueryFirstOTHRFlowEngineData(compID, flowCode, flowSN, out nextRow, out message))
            {
                nextIsLastFlow = nextRow["FlowEndFlag"].ToString() == "1";
                var signLine = nextRow["SignLineDefine"].ToString(); // 1:行政 2:功能 3: 特殊人員
                isSuccess = QueryToUserData(compID, otEmpID, organID, flowOrganID, queryDate, signLine, nextRow, true, out toUserData, out message);
            }
        }
        catch (Exception ex)
        {
        ...
        out Dictionary<string, string> toUserData, out string flowCode, out string flowSN, out string signLineDefine, out bool isLastFlow, out bool nextIsLastFlow, out string message)
    {
        ...
        try
        {
            if (QueryNextOTHRFlowEngineData(compID, flowCaseID, otModel, out thisLogRow, out logdt, out dt, out index, out nextRow, out flowCode, out flowSN, out signLineDefine, out message))
            {
                nextIsLastFlow = nextRow["FlowEndFlag"].ToString() == "1";
    ...
    /// <param name="message">string</param>
    /// <returns>bool</returns>
    private static bool QueryFirstOTHRFlowEngineData(string compID, string flowCode, string flowSN, out DataRow nextRow, out string message)
    {
        var isSuccess = false;
        var systemID = "OT";
        message = "";
        ...
        if (QueryHRFlowEngineDatas(compID, systemID, flowCode, flowSN, out retrunTable, out message) && retrunTable.Rows.Count > 0)
        {
            nextRow = retrunTable.Rows[0];
            isSuccess = true;
        }
        else
        {
            nextRow = retrunTable.NewRow();
            message = "無流程關卡資料!!";
        }
        return isSuccess;
    }
    ...
    /// <param name="flowCaseID">流程系統ID</param>
    /// <param name="otModel">A:預先申請 D:事後申報</param>
    /// <param name="retrunTable">列出關卡</param>
    ...
    /// <param name="message">string</param>
    /// <returns>bool</returns>
    private static bool QueryNextOTHRFlowEngineData(string compID, string flowCaseID, string otModel, out DataRow retrunRow, out DataTable retrunTable, out DataTable retrunTable2, out int thisRowIndex, out DataRow nextRow, out string flowCode, out string flowSN, out string signLineDefine, out string message)
    {
        var isSuccess = false;
        var systemID = "OT";
        message = "";
        ...
        retrunTable2 = new DataTable();
        thisRowIndex = 0;
        retrunRow = retrunTable.NewRow();
        if (QueryHROverTimeLogLast(flowCaseID, otModel, out retrunTable, out retrunRow, out message))
        {
            flowCode = retrunRow["FlowCode"].ToString();
            flowSN = retrunRow["FlowSN"].ToString();
            signLineDefine = retrunRow["SignLine"].ToString();
            var flowSeq = retrunRow["FlowSeq"].ToString();
            if (QueryHRFlowEngineDatas(compID, systemID, flowCode, flowSN, out retrunTable2, out message) && retrunTable2.Rows.Count > 0)
            {
                var thisRow = retrunTable2.Rows.Cast<DataRow>().Where(row => row.Field<string>("FlowSeq") == flowSeq).FirstOrDefault();
                thisRowIndex = retrunTable2.Rows.IndexOf(thisRow);
                if (thisRowIndex < retrunTable2.Rows.Count - 1)
                {
                    nextRow = retrunTable2.Rows[thisRowIndex + 1];
                    isSuccess = true;
                }
                else
                {
                    nextRow = retrunTable2.NewRow();
                    message = "無下一個流程關卡資料!!";
                }
            }
            else
    ...
    /// <returns>bool</returns>
    private static bool QueryOTFlowData(string compID, string empID, string flowCodeFlag, string deptID, string organID, string businessType,
        ...
        out string meassge)
    {
        ...
        flowSN = QueryOTEmpFlowSN(compID, empID, flowCode);
        var retrunTable = new DataTable("RetrunTable");
        var retrunTable2 = new DataTable("RetrunTable2");
        if (!string.IsNullOrEmpty(flowSN) &&
            QueryHRFlowEmpDefineDatas(compID, "OT", flowCode, flowSN, out retrunTable, out meassge) &&
            string.IsNullOrEmpty(meassge) &&
            QueryHRFlowEngineDatas(compID, "OT", flowCode, flowSN, out retrunTable2, out meassge) &&
            retrunTable2.Rows.Count > 0 &&
            string.IsNullOrEmpty(meassge))
        {
            isSuccess = true;
        }
        ...
        {
            isSuccess = QueryOTHRFlowEmpDefineDatas(flowCode, empID, compID, deptID, organID, businessType, rankID, titleID, empFlowRemark, positionID, workTypeID, out flowSN, out meassge);
        }
        return isSuccess;
    }
    ...
    private static string QueryOTEmpFlowSN(string compID, string empID, string flowCode)
    {
        var result = "";
        var isSuccess = false;
        var message = "";
        var systemID = "OT";
        var retrunTable = new DataTable("RetrunTable");
        isSuccess = QueryEmpFlowSN(compID, empID, systemID, flowCode, out retrunTable, out message);
        if (isSuccess && string.IsNullOrEmpty(message))
        {
            var rowData = retrunTable.Rows[0];
    ...
    /// <returns>bool</returns>
    private static bool QueryOTHRFlowEmpDefineDatas(string flowCode, string empID, string flowCompID, string deptID, string organID, string businessType,
        string rankID, string titleID, string empFlowRemark, string positionID, string workTypeID, out string flowSN, out string message)
    {
        ...
        {
            var retrunTable = new DataTable("RetrunTable");
            if (QueryHRFlowEmpDefineDatas("OT", flowCode, flowCompID, deptID, organID, empID, businessType, rankID, titleID, empFlowRemark,positionID, workTypeID, out retrunTable, out message))
            {
                //篩選關卡設定有誤或沒有的
                for (var i = (retrunTable.Rows.Count - 1); i >= 0; i--)
                {
                    var isDataOK = false;
                    var itemData = retrunTable.Rows[i];
                    if (itemData != null && itemData["FlowSN"] != null && itemData["FlowSN"].ToString() != "")
                    {
                        var sFlowSN = itemData["FlowSN"].ToString();
                        var retrunTable2 = new DataTable("RetrunTable2");
                        if (QueryHRFlowEngineDatas(flowCompID, "OT", flowCode, sFlowSN, out retrunTable2, out message) && retrunTable2.Rows.Count > 0)
                        {
                            int flowStartFlagCount = retrunTable2.Rows.Cast<DataRow>().Where(row => row.Field<string>("FlowStartFlag") == "1").Count();
                            int flowEndFlagCount = retrunTable2.Rows.Cast<DataRow>().Where(row => row.Field<string>("FlowEndFlag") == "1").Count();
                    ...
                    if (!isDataOK)
                    {
                        retrunTable.Rows.RemoveAt(i);
                    }
                }

                if (retrunTable.Rows.Count > 0)
                {
                    var rowData = retrunTable.Rows.Cast<DataRow>().Where(row => row.Field<string>("EmpID") == empID).FirstOrDefault();
                ...
                if (!isSuccess)
                {
                    var rowData = retrunTable.Rows.Cast<DataRow>().Where(row => row.Field<string>("PrincipalFlag") == "1").FirstOrDefault();
                    if (rowData != null && rowData["FlowSN"] != null && rowData["FlowSN"].ToString() != "")
                    {
                        flowSN = rowData["FlowSN"].ToString();
                        isSuccess = true;
    ...
    /// <param name="message">return: message</param>
    /// <returns>bool</returns>
    private static bool QueryEmpFlowSN(string compID, string empID, string systemID, string flowCode, out DataTable retrunTable, out string message)
    {
        var isSuccess = false;
        retrunTable = new DataTable();
        message = "";
    ...
    /// <param name="message">return: message</param>
    /// <returns>bool</returns>
    private static bool QueryHRFlowEmpDefineDatas(string compID, string systemID, string flowCode, string flowSN, out DataTable retrunTable, out string message)
    {
        var isSuccess = false;
        retrunTable = new DataTable();
        message = "";
    ...
    private static bool QueryHRFlowEmpDefineDatas(string systemID, string flowCode, string flowCompID, string deptID, string organID, 
        ...
        string positionID, string workTypeID, out DataTable retrunTable, out string message)
    {
        var isSuccess = false;
        retrunTable = new DataTable();
        message = "";
    ...
    /// <returns>bool</returns>
    public static bool QueryHRFlowEngineDatas(string compID, string systemID, string flowCode, string flowSN, out DataTable retrunTable, out string message, string speComp = "", string speEmpID = "")
    {
        var isSuccess = false;
        retrunTable = new DataTable();
        message = "";
