...
public partial class AclExpress_Admin_AclAdminRuleArea : SecurePage
{
    #region 頁面共用
    ...
    //主檔鍵值欄位
    private string[] _Main_KeyList = "RuleID,AreaID".Split(',');
    //明細檔鍵值欄位
    private string[] _Detail_KeyList = "RuleID,AreaID,GrantID".Split(',');
    ...
    private Dictionary<string, string> _Dic_RuleData
    {
        get
        {
            if (ViewState["_Dic_RuleData"] != null)
            {
                return (Dictionary<string, string>)ViewState["_Dic_RuleData"];
            }
            else
            {
                ViewState["_Dic_RuleData"] = Util.getDictionary(AclExpress.getAclRuleData(), 0, 1, true);
                return (Dictionary<string, string>)ViewState["_Dic_RuleData"];
            }
        }
    }
    ...
    protected void Refresh(bool InitGridMain = false, bool InitGridDetail = false)
    {
        ...
        ucGridMain.ucDataQrySQL = "Select * From viewAclAdminRuleArea ";
        ...
        ucGridMain.ucDataGroupKey = "RuleID";
        ...
        ucGridDetail.ucDataQrySQL = string.Format("Select * From viewAclAdminRuleAreaGrantList Where RuleID = '{0}' and AreaID = '{1}' ", (string.IsNullOrEmpty(_RuleID)) ? "---" : _RuleID, (string.IsNullOrEmpty(_AreaID)) ? "---" : _AreaID);
    ...
    protected void fmMain_DataBound(object sender, EventArgs e)
    {
        ...
        if (fmMain.CurrentMode == FormViewMode.Insert)
        {
            ...
            oDdl.ucSourceDictionary = _Dic_RuleData;
    ...
    protected void btnMainUpdate_Click(object sender, EventArgs e)
    {
        ...
        Dictionary<string, string> oDic = Util.getControlEditResult(fmMain);
        sb.Reset();
        sb.AppendStatement("Update AclAdminRuleArea Set ");
        ...
        try
        {
            if (db.ExecuteNonQuery(sb.BuildCommand()) >= 0)
            {
                ...
                dicKey.Add("RuleID", _RuleID);
                dicKey.Add("AreaID", _AreaID);
                AclExpress.IsAclTableLog("AclAdminRuleArea", dicKey, LogHelper.AppTableLogType.Update);
    ...
    protected void btnDetailInsert_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(_RuleID))
        {
            ...
            sb.Reset();
            //新增明細
            sb.AppendStatement("Insert AclAdminRuleAreaGrantList ");
            sb.Append("(RuleID,AreaID,GrantID,Remark,UpdUser,UpdDateTime) ");
            ...
            try
            {
                ...
                dicKey.Add("AreaID", _AreaID);
                dicKey.Add("GrantID", _GrantID);
                AclExpress.IsAclTableLog("AclAdminRuleAreaGrantList", dicKey, LogHelper.AppTableLogType.Create);
    ...
    protected void btnDetailUpdate_Click(object sender, EventArgs e)
    {
        ...
        sb.Reset();
        //更新明細
        sb.AppendStatement("Update AclAdminRuleAreaGrantList Set ");
        sb.Append("  Remark      = ").AppendParameter("Remark", oDic["Remark"]);
        sb.Append(", UpdUser     = ").AppendParameter("UpdUser", UserInfo.getUserInfo().UserID);
        sb.Append(", UpdDateTime = ").AppendDbDateTime();
        sb.Append(" Where 0=0 ");
        ...
        try
        {
            ...
            dicKey.Add("AreaID", _AreaID);
            dicKey.Add("GrantID", _GrantID);
            AclExpress.IsAclTableLog("AclAdminRuleAreaGrantList", dicKey, LogHelper.AppTableLogType.Update);
