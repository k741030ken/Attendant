...
$.extend(Datepicker.prototype, {
	...
	_autoSize: function(inst) {
		if (this._get(inst, 'autoSize') && !inst.inline) {
			...
			var dateFormat = this._defaultDateFormat(inst);
	...
	_selectMonthYear: function(id, select, period) {
        ...
        if( period == 'Y' && this._get(inst, 'ShowROCYear') ) inst.drawYear += 1911;
		this._notifyChange(inst);
		this._adjustDate(target);
	},
    ...
    _defaultDateFormat: function(inst) {
        return $.isArray(this._get(inst, 'dateFormat')) ? this._get(inst, 'dateFormat')[0] : this._get(inst, 'dateFormat');
    },
	...
	_updateAlternate: function(inst) {
		...
		if (altField) { // update alternate field too
			var altFormat = this._get(inst, 'altFormat') || this._defaultDateFormat(inst);
	...
	parseDate: function (format, value, settings) {
		...
		var showROCYear = (settings ? settings.showROCYear : null) || this._defaults.ShowROCYear;
		...
		else if (showROCYear && year < 1911)
            year += 1911;
		else if (showROCYear && year > 1911)
            year -= 1911;
		else if (year < 100 && !showROCYear)
				...
				if (literal)
					...
					switch (format.charAt(iFormat)) {
						...
						case 'M':
							...
							break;
						case 'y':
                            if( showROCYear ) date.setYear(date.getYear() + 1911);
							output += (lookAhead('y') ? (showROCYear ? date.getFullYear() - 1911 : date.getFullYear()) :
	...
	_setDateFromField: function(inst, noDefault) {
		...
		try {
			date = this.parseDate(this._get(inst, 'dateFormat'), dates, settings) || defaultDate;
		} catch (event) {
			this.log(event);
		...
		if ( !inst.yearshtml ) {
			...
			if (secondary || !changeYear)
				...
				for (; year <= endYear; year++) {
					inst.yearshtml += '<option value="' + ( this._get(inst, 'ShowROCYear') ? year - 1911 : year) + '"' +
						(year == drawYear || year == drawYear + 1911 ? ' selected="selected"' : '') + ' title="' + year + '"' +
						'>' + ( this._get(inst, 'ShowROCYear') ? (year - 1911 > 0 ? this._get(inst, 'ROCTitle').replace("{0}", (year == 1912 ? '元' : (year - 1911))) : this._get(inst, 'ROCBTitle').replace("{0}", (1912 - year))) : year) + '</option>';
				}
	...
	_getFormatConfig: function(inst) {
		...
		shortYearCutoff = (typeof shortYearCutoff != 'string' ? shortYearCutoff :
			new Date().getFullYear() % 100 + parseInt(shortYearCutoff, 10));
		return {shortYearCutoff: shortYearCutoff, showROCYear: this._get(inst, 'ShowROCYear'),
	...
	_formatDate: function(inst, day, month, year) {
		...
		return this.formatDate(this._defaultDateFormat(inst), date, this._getFormatConfig(inst));
	}
});
